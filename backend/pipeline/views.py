...
class DashboardStatsView(APIView):
    ...
    Teacher/admin-only. Returns today's snapshot counts.

    NOTE: 'Avg Time Taken' from the UI mock is intentionally omitted —
    StudentSubmission only stores submitted_at, not an exam start time,
    so a per-student duration can't be computed accurately with the
    current schema. Add a `started_at` field to StudentSubmission first
    if this metric is needed later.
    """
    permission_classes = [IsAuthenticated, IsTeacherUser]
...
class AdminRankingsView(APIView):
    ...
    Teacher/admin-only. Returns top-N students ranked by score.
    """
    permission_classes = [IsAuthenticated, IsTeacherUser]
...
class AdminReportsView(APIView):
    ...
    Teacher/admin-only. Returns aggregated attendance + performance
